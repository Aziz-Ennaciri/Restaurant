    ...
    public function stepTwo(Request $request)
{
    ...
    // Ensure $reservation->res_date is a Carbon instance
    $reservationDate = Carbon::parse($reservation->res_date);

    $res_table_ids = Reservation::orderBy('res_date')
        ->get()
        ->filter(function ($value) use ($reservationDate) {
            return Carbon::parse($value->res_date)->format('Y-m-d') == $reservationDate->format('Y-m-d');
        })
        ->pluck('table_id');
